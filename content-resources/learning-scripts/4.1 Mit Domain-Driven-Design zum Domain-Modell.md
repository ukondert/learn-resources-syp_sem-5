...
package "Ausleih-Kontext (Core Domain)" {
    ...
    class Ausleiher <<Aggregate>> AGGREGATE_COLOR {
        --
        +reserviereBuch(exemplarId: UUID): void
    }

    class Reservierung <<Entity>> ENTITY_COLOR
    class AusleihExemplar <<Aggregate>> AGGREGATE_COLOR
    
    class AusleihDomainService <<Domain Service>> SERVICE_COLOR {
        +validiereAusleihe(...): void
    }
    ...
    AusleiherRepository ..> Ausleiher : lädt/speichert
    
    AusleihDomainService ..> Ausleiher : prüft
    AusleihDomainService ..> AusleihExemplar : prüft
}

package "Application Layer" {
    class ReservierungsApplicationService <<Application Service>> SERVICE_COLOR {
        +reserviereBuch(ausleiherId: UUID, exemplarId: UUID): void
    }
    
    ReservierungsApplicationService ..> AusleiherRepository : nutzt
    ReservierungsApplicationService ..> AusleihDomainService : nutzt
}
@enduml
```
...
package "Ausleih-Kontext (Core Domain)" {
    ...
    AusleiherRepository ..> Ausleiher : lädt/speichert
    AusleihExemplarRepository ..> AusleihExemplar : lädt/speichert
