...
## 1.4 Moderne Implementierungs- und Designkonzepte

<!-- Skizze: Einleitung in die Konzepte, die den Übergang von abstrakten Anforderungen (Pflichtenheft) zur technischen Code-Struktur definieren. Fokus auf Trennung von Backend-Logik (DDD) und Frontend-UI (CDD). -->

### 1.4.1 Domain-Driven Design (DDD) – Fokus Backend

<!-- Skizze: Erläuterung, wie Software die Geschäftswelt durch eine fachliche Modellierung (Domain) abbildet. Wichtige Begriffe: Ubiquitous Language, Bounded Context und die Aufteilung in Entities/Value Objects. -->

### 1.4.2 Component-Driven Design (CDD) – Fokus Frontend

<!-- Skizze: Erläuterung des Bottom-Up-Ansatzes im Frontend. Entwicklung von isolierten, wiederverwendbaren UI-Komponenten (Atome, Moleküle). Erwähnung von Tools wie Storybook und der Vorteil für die Testbarkeit und Konsistenz. -->
